...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
	...
	//   http://planning.cs.uiuc.edu/node99.html
      ...
      for (int i=0; i < num_particles; i++)
      {
        ...
        {
          float x_land = map_landmarks.landmark_list[k].x_f;
          float y_land = map_landmarks.landmark_list[k].y_f;
        ...
        for (int l = 0; l < map_observations.size(); l++)
        {
          ...
          int prediction_id = map_observations[l].id;

          for (int m=0; m< landmarks_in_range.size(); m++)
          {
            if (prediction_id == landmarks_in_range[m].id)
            {
